...
public class UtilityClass {
    public static void initialiseLog4j() {
        DOMConfigurator.configure(Constant.LOG4J_XML);
    }
    ...
    public static void updateDbStatusAndRetry(ResultSet resultSet, int retry,
        ...
        if (resultSet != null) {
            if (retry < Constant.MAX_RETRY) {
                ...
                DatabaseUtil.updateDatabase("reason", e.getMessage().replace(
                        "'", ""), id);
                retry = retry + 1;
                DatabaseUtil.insertDataIntoDb(Constant.SQL_WORKITEM,
